...
Persisted proposal evidence uses its own versioned schema. Every evidence record must carry an exact affected point ID. Missing, malformed, or unknown IDs fail closed. One bounded recursive classifier handles both current payloads and persisted evidence. It inspects nested mappings and lists for existing profile/fact identity markers and normalized identity-sensitive keys covering email, phone, address, username/handle, identity names, and national/passport/tax identifiers. Exceeding its depth or item bounds classifies the value as sensitive. If any nested part is identity-bearing, the complete point snapshot or evidence record is replaced before hashing or rendering, summary/status reasons are suppressed, and drift is conservatively `unknown`.
